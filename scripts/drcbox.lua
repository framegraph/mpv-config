...
local function toggle_drcbox()
  if string.find((mp.get_property("af") or ""), "@dynaudnorm") then
    drcbox_enabled = false
  else
    drcbox_enabled = true
  end
  updateAF()
...
mp.add_forced_key_binding(key_toggle_bindings, "drcbox-settings", toggle_bindings)
mp.add_forced_key_binding(key_reset_drcbox, "reset-drcbox", reset_drcbox)
if bindings_enabled then toggle_bindings(true, true) end


mp.register_event("file-loaded", function()
    local af = mp.get_property("af") or ""
	if drcbox_enabled and not string.find(af, "@dynaudnorm") then
		updateAF()
	elseif string.find(af, "@dynaudnorm") then
		drcbox_enabled = true
        local filter = string.sub(string.match(af, "dynaudnorm=[^,]*"), 12)
