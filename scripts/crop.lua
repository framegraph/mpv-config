...
function crop_video(x1, y1, x2, y2)
    ...
    elseif active_mode == "hard" or active_mode == "delogo" then
        ...
        local set_locally = ""
        if active_mode == "delogo" and hw_current:find("%-copy") == nil and hw_current ~= "no" and hw_current ~= "crystalhd" and hw_current ~= "rkmpp" then
            set_locally = "file-local-options/"
            mp.set_property("file-local-options/hwdec", "auto-copy-safe")
            msg.info("Hardware decoding was temporarily switched to copy-back mode to apply delogo filter")
        end
        ...
        mp.set_property_native(set_locally.."vf", vf_table)
    end
end
