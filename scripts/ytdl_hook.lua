local builtin_ytdl_enabled = mp.get_property_bool('ytdl')
if builtin_ytdl_enabled then
    local err = "You must disable the built-in ytdl_hook with --no-ytdl in your configuration!"
    mp.osd_message(err, 5)
    mp.msg.error(err)

    -- This may break, but since we can, let's try to just disable the builtin ytdl.
    mp.set_property_bool('ytdl', false)
end
...
local function formats_to_edl(json, formats, use_all_formats)
    ...
    for index = #formats, 1, -1 do
        ...
        if track.protocol and track.protocol:match("^m3u8") and not json.is_live then
            table.remove(hdr, 2) -- no_clip doesn't work with HLS very well, but is mandatory for livestream playback 
        end
        local skip = #tracks == 0
        local params = ""
...
local function run_ytdl_hook(url)
    local pre_result = mp.get_property_native("user-data/mpv/ytdl/json-subprocess-result")
    ...
    local result
    if not pre_result or type(pre_result) ~= "table" then
        ...
        msg.debug("Using pre-parsed youtube-dl result")
        result = pre_result
    end
