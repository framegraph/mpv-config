...
function remove_cropdetect(keep_label)
    for _, filter in pairs(mp.get_property_native("vf")) do
        if filter.label == cropdetect_label then
            ...
            if keep_label then
                mp.command("no-osd vf add @" .. cropdetect_label .. ":null")
            end
            break
        end
    end
...
function detect_end()
    ...
    if cropdetect_metadata then
        ...
        mp.command("no-osd vf add @fail:null")
...
function apply_crop(meta)
    ...
    if not is_effective then
        ...
        mp.command("no-osd vf remove @" .. cropdetect_label)
        return
    end
