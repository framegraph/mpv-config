...
--END OF SAMPLE REPLACE ]=====]
  ...
  --Playlist name prefix, useful for pinning the playlist file in folders with default alphabetical sorting
  ...
  -- Опции для функционала сборки SearchDownload, настраиваются в mpv.conf сборки
  per_playlist_properties = "",
  revert_after_playlist_closing = true
}
...
local skip_initial_msg = false
local ours_playlist_opened = false
local prop_table = {}
...
function save_playlist(filename, with_properties)
  if settings.per_playlist_properties == "" then with_properties = false end
  ...
  if string.find(mp.get_property('path'), '://') then
	if settings.playlist_savepath == nil or settings.playlist_savepath == "" then
        ...
        if not savepath or savepath == "" then savepath = mp.get_property("working-directory") end
	else
  ...
  if name == nil then
	...
	if string.find(path, '://') then --для файлов по сети не можем получить название папки - берём название первого файла в плейлисте
		pl_title = mp.get_property("playlist/0/title") or mp.get_property("media-title")
        pl_title = pl_title:gsub("[/\\:%*%?\"<>|]", " "):gsub("%s+", " "):gsub("^%s+", ""):gsub("%s+$", "")
	else
        path = path:gsub("/", "\\")
		local second_last_backslash = string.find(path, "\\[^\\]*\\[^\\]*$") or 0
		local last_backslash = string.find(path, "\\[^\\]*$")
		pl_title = string.sub(path, second_last_backslash + 1, last_backslash - 1):gsub(":", "")
	end
    name = settings.playlist_name_prefix .. pl_title .. ".m3u"
  end

  local savepath = utils.join_path(savepath, name):gsub("[/\\]", package.config:sub(1,1))
  ...
  if not file then
    ...
    if with_properties then
      file:write("#mpv_properties:" .. get_properties_json() .. "\n")
    end
    local i=0
    while i < length do
    ...
    if with_properties then
      saved_msg = "Playlist with current state written to: "..savepath
      local changed_props = get_changed_props_list()
      if changed_props ~= "" then
        saved_msg = saved_msg .. "\nProperties changed since mpv start: " .. changed_props
      else
        saved_msg = saved_msg .. "\nAll saved properties are the same as at mpv start"
      end
      skip_initial_msg = true
    else
      ours_playlist_opened = false
    end
    if settings.display_osd_feedback then mp.osd_message(saved_msg) end
    msg.info(saved_msg)
    file:close()
    mp.commandv("script-message-to", "auto_save_state", "playlist-created", savepath, name) -- для функционала сборки SearchDownload: сразу же наблюдаем созданный плейлист скриптом auto-save-state
  end
end

function get_properties_json()
    local props = {}
    for prop_name in pairs(prop_table) do
        props[prop_name] = mp.get_property_native(prop_name)
    end
    return utils.format_json(props)
end

function get_changed_props_list(set_props)
    local changed_props = {}
    for prop, orig_val in pairs(prop_table) do
        local curr_val = mp.get_property_native(prop)
        if not is_equal(curr_val, orig_val) then
            table.insert(changed_props, prop)
            if set_props then
                mp.set_property_native(prop, orig_val)
            end
        end
    end
    return table.concat(changed_props, ", ")
end

function is_equal(p1, p2)
    if type(p1) == "table" then
        return utils.to_string(p1) == utils.to_string(p2)
    elseif type(p1) == "number" then
        return math.floor(p1 * 10^5 + 0.5) == math.floor(p2 * 10^5 + 0.5) -- не учитываем ошибку округления при сравнении
    else
        return p1 == p2
    end
end
...
function sortplaylist(mode, startover)
  local playlist = mp.get_property_native('playlist')
  if #playlist < 2 then return end

  local current_mode = sort_mode
  if mode and tonumber(mode) then
    current_mode = tonumber(mode)
  end
  local is_remote = is_protocol(mp.get_property("path")) -- сортируем онлайн-видео по названиям в плейлисте, а локальные - по путям к файлам
  local order = {}
  for i=1, #playlist do
		order[i] = i
    playlist[i].string = get_name_from_index(i - 1, not is_remote)
	end

  table.sort(order, function(a, b)
    return sort_modes[current_mode].sort_fn(a, b, playlist)
  end)
  ...
  if settings.display_osd_feedback then
    mp.osd_message("Playlist sorted with "..sort_modes[current_mode].title)
  end
end
...
function handlemessage(msg, value, value2)
  ...
  if msg == "sort" then sortplaylist(value, value2) ; return end
...
mp.register_script_message("save-playlist-and-properties", function() save_playlist(nil, true) end)

mp.register_script_message("current-playlist", function(pl_path) -- получаем текущий открытый файл плейлиста от скрипта auto-save-state
    if skip_initial_msg then
        ours_playlist_opened = true
        skip_initial_msg = false
        return
    end

    if pl_path and pl_path ~= "" then
        -- пробуем прочитать плейлист, и если в нём есть данные о сохранённых свойствах - восстанавливаем их
        local pl = io.open(pl_path, "r")
        if pl then
            pl:read("*l") -- #EXTM3U
            local line = pl:read("*l") -- возможно, сохранённые свойства
            if line and line:find("^#mpv_properties:") then
                local saved_props, err = utils.parse_json(line:match("^#mpv_properties:(.*)"))
                if saved_props then
                    ours_playlist_opened = true
                    local changed_props = {}
                    for prop in settings.per_playlist_properties:gmatch("[^,%s]+") do -- восстанавливаем только те свойства, которые заданы в настройках в текущий момент
                        if saved_props[prop] ~= nil then
                            local curr_val = mp.get_property_native(prop)
                            if not is_equal(curr_val, saved_props[prop]) then
                                table.insert(changed_props, prop)
                                mp.set_property_native(prop, saved_props[prop])
                            end
                        end
                    end
                    if #changed_props > 0 then
                        local props_str = "Restored: " .. table.concat(changed_props, ", ") -- названия свойств всё равно на английском
                        if settings.display_osd_feedback then mp.osd_message(props_str) end
                        msg.info(props_str)
                    end
                else
                    msg.error("Failed to parse mpv properties saved in '" .. pl_path .. "', reason: "..(err or "unknown"))
                end
            end
            local a4k = pl_path:match("%[A4K(.)%]%.m3u8?$") -- для совместимости с прошлыми версиями сборки: включаем шейдеры Anime4k по тегу в названии плейлиста
            if a4k and a4k:match("[%d%-=]") then
                mp.commandv("keypress", "Ctrl+" .. a4k)
                ours_playlist_opened = "a4k"
            end
            pl:close()
        end
    -- просмотр из плейлиста окончен - но сбрасываем свойства только, если до этого их восстанавливали
    elseif ours_playlist_opened and settings.revert_after_playlist_closing then
        if ours_playlist_opened == "a4k" then
            -- отключаем ранее включённые по тегу шейдеры Anime4k (плейлист обычного формата, поэтому никаких других свойств изменено не было)
            mp.set_property_native("glsl-shaders", {})
            mp.osd_message("Playlist has changed - GLSL shaders cleared")
        else
            local changed_props = get_changed_props_list(true)
            if changed_props ~= "" then
                changed_props = "Playlist has changed - reverted: " .. changed_props
                if settings.display_osd_feedback then mp.osd_message(changed_props) end
                msg.info(changed_props)
            end
        end
        ours_playlist_opened = false
    end
...
-- получаем изначальные значения свойств, строго после восстановления свойств скриптом persist-properties и до окрытия каких-либо плейлистов
local props_obtained = false
mp.add_hook("on_before_start_file", 50, function()
    if not props_obtained then -- в mpv нет функции mp.remove_hook
        for prop in settings.per_playlist_properties:gmatch("[^,%s]+") do
            local prop_val, err = mp.get_property_native(prop)
            if err and err == "property not found" then -- отбрасываем несуществующие свойства
                mp.msg.error(prop .. " - no such property!")
            else
                prop_table[prop] = prop_val
            end
        end
        props_obtained = true
        msg.debug("Initial property values: " .. utils.to_string(prop_table))
    end
end)