...
--Changes are recommended to be made in the script-opts directory.
	...
	text_time_type = 'duration', --The time type for items on the list. Select between 'duration', 'length', 'remaining', 'duration/length'.
	...
	--in the first parameter, you can define from the available styles: default, default-compact, hms, hms-full, timestamp, timestamp-concise. "default" to show in HH:MM:SS.sss format. "default-compact" is the same as default but truncates the hours when they are 0. "hms" to show in 1h 2m 3.4s format. "hms-full" is the same as hms but keeps the hours and minutes persistent when they are 0. "timestamp" to show the total time as timestamp 123456.700 format. "timestamp-concise" shows the total time in 123456.7 format (shows and hides decimals depending on availability).
    ...
    --The minimum interval in hours that must pass before updating the watch history backup file. Set it to 0 to disable automatic backups
    backup_interval=24,

    --Absolute path to an existing directory for saving backups. Leave as empty to use the same directory as the history log
    backup_dir=[[]],

    --The minimum number of entries in the history required to create an automatic backup, so that if the original file is corrupted, the newly created log does not overwrite the previously made backup
    min_entries_for_backup=50
...
orig_lower = string.lower
string.lower = function(str) -- перенаправление всех функций для приведения регистра букв к нижнему на аналогичную, но с поддержкой кириллицы (твик от SearchDownload)
    str = str:gsub("[\208][\129\144-\175]", function(chr) -- рассматриваем последовательности байтов, соответствующие заглавным русским буквам в UTF-8
        local second_byte = chr:byte(2)
        if second_byte == 129 then -- буква Ё кодируется по особым правилам
            return "ё"
        elseif (second_byte + 32 > 191) then -- для символов длиной 2 байта возможный диапазон значений второго байта от 128 до 191 
            return string.char(209, second_byte - 32) -- + 32 - 192 + 128
        else
            return string.char(208, second_byte + 32)
        end
    end)
    return orig_lower(str) -- вызываем стандартную функцию для приведения регистра букв к нижнему (работает только с латинскими буквами)
end
...
if o.backup_dir == "" then
    backup_path = log_fullpath .. ".backup"
else
    backup_path = utils.join_path(o.backup_dir, o.log_file .. ".backup")
end
...
function contain_value(tab, val)
	...
	for index, value in ipairs(tab) do
		if value.match(orig_lower(val), orig_lower(value)) then
			return true
		end
	end
...
function has_value(tab, val, array2d)
	...
	if not array2d then
		for index, value in ipairs(tab) do
			if orig_lower(value) == orig_lower(val) then
				return true
			end
		end
	end
	if array2d then
		for i=1, #tab do
			if tab[i] and orig_lower(tab[i][array2d]) == orig_lower(val) then
				return true
			end
		end
	end
...
function format_time(seconds, sep, decimals, style)
	...
	if not style or style == '' or style == 'default' or (style == 'default-compact' and h > 0) then
		sep = sep and sep or ":"
		return string.format("%02d"..sep.."%02d"..sep..second_format, h, m, s)
	elseif style == 'default-compact' then
        sep = sep and sep or ":"
...
function get_file()
    ...
    local curr_playlist_prop = nil
    if current_playlist == "" then -- пробуем также получить текущий файл плейлиста из специального свойства и используем, если оно доступно
        local playlist_property = mp.get_property("playlist/0/playlist-path")
        if playlist_property and playlist_property ~= "" and not playlist_property:find("://") and playlist_property:find("%.%w+$") then
            curr_playlist_prop = playlist_property
        end
    end

    if (current_playlist and current_playlist ~= "") or curr_playlist_prop then
        path = curr_playlist_prop or current_playlist
        local state = ""
...
function read_log_table()
	...
	return read_log(function(line)
		...
		if test then
            ...
            if not p then return end -- corrupted history log line
			d, n, e = p:match('^(.-)([^\\/]-)%.([^\\/%.]-)%.?$')
		end
		dt = line:match('%[(.-)%]')
		t = line:match(' | ' .. esc_string(log_time_text) .. '(%d*%.?%d*)(.*)$')
		ln = line:match(' | ' .. esc_string(log_length_text) .. '(%d*%.?%d*)(.*)$')
        if not ln or not t or not p then return end
...
function get_list_contents(filter, sort)
    ...
    -- вместо того, чтобы считывать из файла и парсить историю просмотра при каждом переключении фильтра и каждой введённой букве в поиск,
    -- делаем это только при открытии меню (но каждый раз, чтобы была синхронизация между одновременно открытыми окнами плеера)
    if full_list_contents ~= nil and full_list_contents[1] then
        list_contents = full_list_contents
    else
        list_contents = read_log_table()
        full_list_contents = list_contents
    end
	...
	if filter == 'recents' then
		...
		for i = list_total, 1, -1 do
            local lowered_path = orig_lower(list_contents[i].found_path)
			if not unique_values[lowered_path] then
				unique_values[lowered_path] = 1
	...
	if filter == 'distinct' then
		...
		for i = list_total, 1, -1 do
			if list_contents[i].found_directory and not unique_values[orig_lower(list_contents[i].found_directory)] and not starts_protocol(protocols, list_contents[i].found_path) then
				unique_values[orig_lower(list_contents[i].found_directory)] = 1
				table.insert(filtered_table, list_contents[i])
			end
		end
		table.sort(filtered_table, function(a, b) return a['found_sequence'] < b['found_sequence'] end)
	...
	if search_active and search_string ~= '' then
		...
		for search in search_string:gmatch("[^%s]+") do
			if search_query ~= '' then
                ...
                search_query = esc_string(search)
            end
		end
		...
		for i = 1, #list_contents do
			...
			elseif o.search_behavior == 'any-notime' then
				contents_string = (list_contents[i].found_title or '')..list_contents[i].found_path
			end
...
function draw_list()
	...
	for i = list_start, list_start + o.list_show_amount - 1, 1 do
		...
		if list_contents[#list_contents - i][osd_time_type] and tonumber(list_contents[#list_contents - i][osd_time_type]) > 0 then
            local osd_time = format_time(list_contents[#list_contents - i][osd_time_type], o.list_time_format[3], o.list_time_format[2], o.list_time_format[1])
            if o.text_time_type == 'duration/length' then
                osd_time = osd_time .. ' / ' .. format_time(list_contents[#list_contents - i]['found_length'], o.list_time_format[3], o.list_time_format[2], o.list_time_format[1])
            end
			osd_msg = osd_msg .. o.time_seperator .. osd_time
		end
...
function display_list(filter, sort, action)
	...
	if not list_contents and not search_active or not list_contents[1] and not search_active then return true end
...
function load(list_cursor, add_playlist, target_time)
	...
	if file_exists(list_contents[#list_contents - list_cursor + 1].found_path) or starts_protocol(protocols, list_contents[#list_contents - list_cursor + 1].found_path) then
		local entry_name = list_contents[#list_contents - list_cursor + 1].found_name or list_contents[#list_contents - list_cursor + 1].found_path
        if not add_playlist then
			if filePath ~= list_contents[#list_contents - list_cursor + 1].found_path then
				file_before_playlist = mp.get_property("path")
                mp.commandv('loadfile', list_contents[#list_contents - list_cursor + 1].found_path)
				resume_selected = true
                if string.find(entry_name, "^⫶☰ ") and not string.find(entry_name, "/1%]$") then
                    playlist_pos = tonumber(string.match(entry_name, "%[(%d+)%/%d+%]$") or "")
                    if playlist_pos then
                        mp.observe_property("playlist-count", "number", change_pl_pos)
                    end
                end
                list_close_and_trash_collection()
			else
			...
			if o.osd_messages == true then
				mp.osd_message('Loaded:\n' .. entry_name .. o.time_seperator .. format_time(seekTime, o.osd_time_format[3], o.osd_time_format[2], o.osd_time_format[1]))
			end
			msg.info('Loaded the below file:\n' .. entry_name .. ' | '.. format_time(seekTime))
		else
...
function delete_log_entry(multiple, round, target_path, target_time, entry_limit, delete_autosaved_entry)
    ...
    if delete_autosaved_entry == true then
        if list_contents[#list_contents].found_path == target_path and tonumber(list_contents[#list_contents].found_time) == 0 then
			table.remove(list_contents, target_index)
			trigger_delete = true
		end
    end
...
function select_filter_sequence(pos)
	...
	if o.loop_through_filters then
		...
		if not target_pos and pos < 0 or target_pos and target_pos < 1 then
            target_pos = #o.filters_and_sequence
		end
	end

	if o.filters_and_sequence[target_pos] then
		local filter_results_empty = display_list(o.filters_and_sequence[target_pos], nil, 'hide-osd')
        if filter_results_empty then
            select_filter_sequence(pos)
        end
	end
end
...
function unbind_search_keys()
    ...
    mp.remove_key_binding('search_word_del')
	if not search_active then
...
function history_blacklist_check()
	...
	elseif not starts_protocol(protocols, filePath) then
        if not filePath:match('^(.-)([^\\/]-)%.([^\\/%.]-)%.?$') then return false end
...
function write_log(target_time, update_seekTime, entry_limit, del_autosaved_entry)
    ...
    -- удаляем дубликаты текущей записи (с чтением актуального лога перед этим), а также проверяем необходимость бэкапа
    -- только при окончании воспроизведения, а при открытии файла лишь добавляем запись в конец лога
    if seekTime > 0 or entry_limit or del_autosaved_entry then
        delete_log_entry(false, true, filePath, math.floor(seekTime), entry_limit, del_autosaved_entry)
        create_backup()
    end
	...
	elseif o.file_title_logging == 'protocols' and (starts_protocol(o.logging_protocols, filePath) or fileTitle:find("^⫶☰ ")) then
		f:write(("[%s] \"%s\" | %s | %s | %s"):format(os.date(o.date_format), fileTitle, filePath, log_length_text .. tostring(fileLength), log_time_text .. tostring(seekTime)))
	elseif o.file_title_logging == 'protocols' and not (starts_protocol(o.logging_protocols, filePath) or fileTitle:find("^⫶☰ ")) then
...
function create_backup()
    -- предполагается, что файл с историей просмотра только что прочитан
    if o.backup_interval > 0 and list_contents and #list_contents > o.min_entries_for_backup then
        local tmp_path = backup_path .. "_tmp"
        local file_info = utils.file_info(backup_path)
        ...
        if not file_info then
            ...
            if f then
                ...
                f:close()
                msg.info("Backup of the watch history has been created")
            else
                if not err then err = "" end
                if o.osd_messages then mp.osd_message("Unable to create a backup file for the watch history!\n" .. err) end
                msg.error("Unable to create a backup file " .. backup_path .. "\nError: " .. err)
            end
        -- иначе сначала сохраняем бэкап во временный файл, чтобы если плеер вылетит в момент записи бэкапа, он не повредился (данные можно будет извлечь из основного файла)
        elseif (os.time() - file_info["mtime"]) > (o.backup_interval * 3600) then
            ...
            if f then
                ...
                msg.info("Backup of the watch history has been updated")
            else
                if not err then err = "" end
                if o.osd_messages then mp.osd_message("Unable to create a backup file for the watch history!\n" .. err) end
                msg.error("Unable to create a backup file " .. tmp_path .. "\nError: " .. err)
            end
        end
    end
end
...
function history_save(target_time, del_autosaved_entry)
	if filePath ~= nil then
		if history_blacklist_check() then
			return
		end
		write_log(target_time, false, o.same_entry_limit, del_autosaved_entry)
		if list_drawn then
			get_list_contents()
			select(0)
		end
...
mp.register_script_message("current-playlist", function(v) -- получаем текущий открытый файл плейлиста от скрипта auto-save-state.lua из сборки SearchDownload
    -- на версии mpv 0.37 появилось свойство для получения файла плейлиста напрямую, однако я хочу сохранить совместимость с более шустрой версией 0.35.1, а также последней стабильной, поддерживающей windows 7
    current_playlist = v
    if mp.get_property("time-pos") and v ~= "" then -- если только что создали плейлист с помощью playlistmanager, сразу же считаем его открытым и сохраняем в истории просмотра
        filePath, fileTitle, fileLength = get_file()
    end
end) 
