...
local opts = {
    ...
    -- Using audio filters with lookahead (loudnorm, dynaudnorm) 
    -- before detection filter breaks silence detection logic
    -- With this option, the detection filter will always be kept last 
    -- in the filter list to avoid that
    -- However, the audio signal will be analyzed after other filters have been applied
    filter_keep_latest = false,
...
local filter_pos = opts.filter_keep_latest and "add" or "pre"
...
local function reapply_filter()
    ...
    reapply_filter_timer = mp.add_timeout(0.4, function()
        ...
        mp.commandv("af", filter_pos, get_silence_filter())
        update_filter_opts()
        update_info_now(now)
...
local function check_time()
    ...
    if not prev_speed or prev_speed >= 100 then return end -- don't allow conflict with skip-to-silence script
...
local function insert_detect_filter()
    if not is_filter_added then
        ...
        if mp.get_property("af"):find("@"..detect_filter_label..":[^!]") then
            ...
            mp.commandv("af", filter_pos, "@"..detect_filter_label..":!anull")
        end
    end
    mp.commandv("af", filter_pos, get_silence_filter())
...
local function update_opts(list)
...
options.read_options(opts, nil, update_opts)
options.read_options(opts, "skip-silence", update_opts)
...
mp.observe_property("user-data/skipsilence/enabled", "bool", function(_, enabled)
    mp.commandv('script-message-to', 'uosc', 'set-button', 'silence-skip', utils.format_json({
        icon = 'speed',
        active = (enabled == true),
        tooltip = (is_rus and 'Пропускать тишину' or 'Skip silence'),
        command = 'script-message-to ' .. mp.get_script_name() .. ' toggle no-osd'
    })..'')
end)

mp.observe_property("af", "string", function(_, af)
    if af and not af:find("@skiptosilence") then -- don't allow conflict with skip-to-silence script
        if af:find("@"..detect_filter_label) then
            if not is_enabled and not opts.filter_persistent then
                enable("no-osd")
            end
            if opts.filter_keep_latest then
                local af_table = mp.get_property_native("af") or {}
                for i, filter in ipairs(af_table) do -- if detect_filter is not last, move it to the end
                    if filter.label == detect_filter_label and #af_table > i then
                        local detect_filter = table.remove(af_table, i)
                        table.insert(af_table, detect_filter)
                        dprint("moving detection filter to the end of filter list")
