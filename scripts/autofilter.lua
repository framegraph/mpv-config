...
o.fix_not_chosen_audio = true -- исправление неприятной особенности плеера: если в предыдущем видео была вручную выбрана аудиодорожка с номером, которого нет в текущем видео,
...
o.load_ytdl_preview = true -- загрузка превью (миниатюры) роликов с ютуба и других видеохостингов, для которых yt-dlp предоставляет ссылку,
                           -- для показа в медиа-панели windows (SMTC), появляющейся при использовании мультимедийных клавиш
o.auto_subs_list = "orig,slang" -- список языков, а также порядок добавления автоматических и автопереведённых субтитров при наличии на роликах с ютуба (через запятую)
                                -- orig - субтитры языка видео,  slang - языки из свойства плеера --slang,  en,de - субтитры на английском и немецком соответственно
...
local ytdl_err_announced = false
...
mp.register_event("file-loaded", function()
    ...
    if o.fix_not_chosen_audio and mp.get_property("aid") == "no" then
    ...
    if o.auto_subs_list == "" and o.load_ytdl_preview == false then return end
    ...
    if o.auto_subs_list ~= "" and data and data.automatic_captions then
        ...
        local subs_list = o.auto_subs_list:gsub("slang", mp.get_property("slang"))
        local orig_lang = nil
        local orig_lang_name = "?"
        -- определение языка оригинала для информативного названия дорожек, и чтобы не загружать одни и те же субтитры дважды
        for k, v in pairs(data.automatic_captions) do
            if k:find("%-orig") then
                orig_lang = k:match("^%w+")
                if v[1] and v[1].name then
                    orig_lang_name = v[1].name:gsub("%s+%(Original%)", "")
                end
            end
        end
        for lang in subs_list:gmatch("[^,%s]+") do
            if orig_lang and (lang == "orig" and data.automatic_captions[orig_lang .. "-orig"]) 
                          or (not subs_list:find("orig") and data.automatic_captions[lang .. "-orig"]) 
            then
                for _, v in ipairs(data.automatic_captions[orig_lang .. "-orig"]) do
                    -- из всех предоставляемых ютубом форматов субтитров (json3, srv[1-3], ttml, vtt) плеер поддерживает лишь vtt (и то без стилей)
                    if v.ext == "vtt" and v.url and v.name then
                        add_subs(v.url, orig_lang_name .. " (автоматические)", orig_lang .. "-auto")
                        break
                    end
                end
            elseif data.automatic_captions[lang] and not (orig_lang and data.automatic_captions[lang .. "-orig"]) then
                for _, v in ipairs(data.automatic_captions[lang]) do
                    if v.ext == "vtt" and v.url and v.name then
                        add_subs(v.url, v.name .. " (автоперевод с " .. orig_lang_name .. ")", lang .. "-autotr")
                        break
                    end
                end
            end
    ...
    if o.load_ytdl_preview then
        local preview_loaded = false
        if vidpath:match("^https?://[^/]*youtu%.?be") then -- загрузка более подходящего превью для ютуб-роликов вручную
            ...
            if youtube_id and #youtube_id == 11 then -- загружаем только, если уверены в допустимости ID ютуб ролика
                -- выбираем изображение для превью в зависимости от соотношения сторон ролика, чтобы занять максимум полезной области
                -- hqdefault - всегда 480x360 (4:3), mqdefault - 320x180 (достаточно для показа в окошке SMTC) (16:9), оба эти превью должны быть всегда доступны
                local w = mp.get_property_number("width") or (data and data.width)
                local h = mp.get_property_number("height") or (data and data.height)
                local variant = "mqdefault.jpg"
                if w and h and w / h < 1.4 then
                    variant = "hqdefault.jpg"
                end
                local preview_path = "https://i.ytimg.com/vi/" .. youtube_id .. "/" .. variant
                mp.commandv("async", "video-add", preview_path, "auto", "Preview", "", "yes")
                preview_loaded = true
            end
        end
        if not preview_loaded and data and data.thumbnail then -- загрузка превью, предоставленного yt-dlp
            mp.commandv("async", "video-add", data.thumbnail, "auto", "Preview", "", "yes")
        end
    end
end)

mp.register_event("start-file", function()
    if o.autoplay and vidpath ~= mp.get_property("path") then -- не снимаем с паузы при перезагрузке того же видео или смене качества
        mp.set_property_bool("pause", false)
    end
    vidpath = mp.get_property("path") or ""
    ytdl_err_announced = false
...
mp.register_event("log-message", function(tab)
    if o.reload_on_network_errors then
        ...
        if tab["prefix"] == "ffmpeg" and tab["text"]:find("HTTP error 403 Forbidden") and mp.get_property("time-pos") then -- только если ошибка возникла уже после удачного открытия
            if vidpath:match("^https?://[^/]*youtu%.?be") then -- ютуб ограничивает срок прямой ссылки 6 часами (может и раньше, если IP сменился)
                osd_msg("Сессия истекла, перезагрузка...")
            else
    ...
    if o.adaptive_hwdec_mode and tab["prefix"] == "vf" and tab["text"]:match("Disabling filter.+because it has failed") then
        ...
        if hw_current:find("%-copy") == nil and hw_current ~= "no" and hw_current ~= "crystalhd" and hw_current ~= "rkmpp" then
            ...
            hw_change_time = os.clock() -- ждём, пока переключится режим декодирования (не больше 5 секунд), и только потом применяем фильтры
    ...
    if tab["prefix"] == "cplayer" and tab["text"]:find("Failed to recognize file format") and not ytdl_err_announced then
        osd_msg("Не удалось распознать формат файла\n" .. vidpath)
    end
    if tab["prefix"] == "stream" and tab["text"]:find("^Failed to open") and not ytdl_err_announced then
        osd_msg("Не удалось открыть '" .. tab["text"]:gsub("^Failed to open%s*", ""):gsub("%.?%s*$", "") .. "'")
    end
    
    if tab["prefix"] == "lavf" and tab["text"]:find("...treating it as fatal error") then
        osd_msg("Фатальная ошибка при демультиплексировании, подробности в консоли плеера")
    end
    if tab["prefix"] == "lavfi" and tab["text"]:find("failed to configure the filter graph") and mp.get_property("lavfi-complex") ~= "" then
        osd_msg("Не удалось применить lavfi-complex фильтр, подробности в консоли плеера")
    end
    if tab["prefix"] == "vapoursynth" and tab["text"]:find("could not init VS") then
        osd_msg("Ошибка при инициализации VapourSynth, подробности в консоли плеера")
    end
end)

if o.adaptive_hwdec_mode then
    mp.register_event("playback-restart", function()
        if vf_action_fn and (os.clock() - hw_change_time) < 5 and mp.get_property("hwdec") == o.hw_copyback then
            vf_action_fn()
        end
        vf_action_fn = nil
    end)
...
function change_temp(val)
    if not val or not tonumber(val) then
        mp.osd_message("Не указан сдвиг цветовой температуры в К")
        return
    end
    
    local lp_opts = mp.get_property_native("libplacebo-opts") or {}
    local curr_temp = tonumber(lp_opts["temperature"] or 0)
    curr_temp = math.min(math.max(curr_temp + val / 3500, -1.14), 5.28)
    if math.abs(curr_temp) < math.abs(val / 3500 / 2) then
        curr_temp = 0
    end
    lp_opts["temperature"] = tostring(curr_temp)
    mp.set_property_native("libplacebo-opts", lp_opts)
    mp.osd_message("Цветовая температура: " .. math.floor(6500 + curr_temp * 3500 + 0.5) .. "K" .. (curr_temp == 0 and " (по умолчанию)" or ""))
end

local default_cache_secs, default_cache_ram
function toggle_cache_mode()
    if not mp.get_property_bool("cache-on-disk") then
        default_cache_secs = mp.get_property_number("cache-secs")
        default_cache_ram = mp.get_property_number("demuxer-max-bytes")
        mp.set_property_bool("cache-on-disk", true)
        mp.set_property_number("cache-secs", 10^9)
        mp.set_property_number("demuxer-max-bytes", 500 * 2^20) -- метаданные (индекс) по-прежнему хранятся в ОЗУ - ставим лимит 500 МБ, чтобы могло поместиться до 7-10 часов видео
        mp.osd_message("Кэширование видео на диск")
    else
        mp.set_property_bool("cache-on-disk", false)
        mp.set_property_number("cache-secs", default_cache_secs or 10^9)
        mp.set_property_number("demuxer-max-bytes", default_cache_ram or 150 * 2^20)
        mp.osd_message("Кэширование в оперативную память")
    end
end
