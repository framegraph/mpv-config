...
local options = {
    ...
    -- Available: sponsor,selfpromo,interaction,intro,outro,preview,music_offtopic,filler 
	categories = '',
    ...
    -- Use the same proxy to fetch categories as for the video itself
    pass_proxy = true
}

opt.read_options(options)
options.categories = options.categories:gsub('^', '"'):gsub('$', '"'):gsub(',', '","')

function skip_ads(name,pos)
	if pos then
		for _, i in pairs(ranges) do
			v = i.segment[2]
			if i.segment[1] <= pos and v > pos and not i.already_skipped then
				...
				mp.osd_message(("[sponsorblock] skipping %s forward %ds"):format(i.category, math.floor(v-mp.get_property("time-pos"))))
...
if options.categories ~= "" and options.categories ~= '""' then
