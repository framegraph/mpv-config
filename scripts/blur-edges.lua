...
function getBind(option, delta)
    return function()  -- действие при нажатии клавиши настройки
...
function set_blur(reappl)
    ...
    -- отключение фильтров, которые нельзя перенести в фильтр подсветки (список неполный)
    vf_list = vf_list:gsub(",?vapoursynth=[^,]*", ""):gsub(",?lavfi=[^,]*", ""):gsub(",?format=[^,]*", ""):gsub(",?@%w+:[^,]*", ""):gsub("^,", "")
    vf_list = vf_list:gsub("%%%d+%%@%d+=", "") --при добавлении фильтра записью вида crop=400:300 mpv преобразует её в формат crop=%2%@0=400:%2%@1=300 который lavfi не распознаёт
    vf_list = vf_list:gsub(",?[^,]*%%[^,]*", ""):gsub("^,", "") --отбрасываем фильтры, в которых всё ещё остался percent-encoding
    ...
    if vf_crop == "no" or vf_list == "" then
        ...
        crop_w = vf_crop:match("w=(%d+)") or vf_crop:match("=(%d+)") or width
        crop_h = vf_crop:match("h=(%d+)") or vf_crop:match(":(%d+)") or height
        crop_x = vf_crop:match("x=(%d+)") or vf_crop:match(":%d+:(%d+)") or (width-crop_w)/2
        crop_y = vf_crop:match("y=(%d+)") or vf_crop:match(":%d+:%d+:(%d+)") or (height-crop_h)/2
        vf_list = vf_list:gsub("crop=", "!$!", 1):gsub(",?crop=[^,]*", ""):gsub("^,", ""):gsub("!$!", "crop=") --убираем все остальные обрезки
    end
    ...
    if mp.get_property("deinterlace") == "auto" and mp.get_property_bool("deinterlace-active") then
        crop_apply = "bwdif," .. crop_apply -- автоматический деинтерлейсинг не работает при включённой подсветке - добавляем фильтр деинтерлейсинга вручную
    end
    crop_apply = crop_apply:gsub(",$", "")
    if crop_apply ~= "" then
    ...
    mp.set_property("hwdec", "no") --во время активации lavfi-complex должен быть выключено HW-декодирование, даже HW-copy
    set_lavfi_complex(filter)
    --чтобы не было проблем с сохранением SW-декодирования скриптом persist-properties при закрытии плеера со вкл подсветкой, сразу же возвращаем оригинальные настройки
    mp.set_property("hwdec", orig_value_hwdec) --плеер не включит HW декодирование во время работы подсветки, а после конца работы включит автоматически
...
function set_autocrop()
    ...
    if not autocrop_installed then
        mp.osd_message("Не установлен модифицированный скрипт autocrop.lua (идёт вместе со сборкой SearchDownload) для автообрезки чёрных полей")
        return
    end
    if active then
        unset_blur()
